...
public class OpenWeatherSecondaryController {
        ...
        if (enable) {
            ...
            try {
                String retSrc = sendGET(
                        "http://api.openweathermap.org/data/3.0/stations/" + station_id + "?APPID=" + APPID);
                @Cleanup
    ...
    public OpenWeatherWeather getWeatherForWeatherDStationCoordinates() {
        if (enable) {
            ...
            try {
                String retSrc = sendGET(
                        "http://api.openweathermap.org/data/2.5/weather?lat=53.894658&lon=12.183633&units=metric&lang=de&appid="
                                + APPID);
